...
namespace coen79_lab8
{
    ...
    void deque<Item>::operator=(const deque<Item>& source) {
        ...
        for (size_type bp_array_index = 0; bp_array_index < source.bp_array_size; ++bp_array_index)
        {
            ...
            else
            {
                ...
                {
                    // STUDENT WORK...completed
                    *(block_pointers[bp_array_index]+block_item_index) = *(source.block_pointers[bp_array_index]+block_item_index);  //gets item at each index inside the block and sets to corresponding block index in copied array
                    if(*(block_pointers[bp_array_index]+block_item_index)==*source.front_ptr){
                        front_ptr = block_pointers[bp_array_index]+block_item_index;
                    }
                    else if(*(block_pointers[bp_array_index]+block_item_index)==*source.back_ptr){
    ...
    deque<Item>::~deque () {
        ...
        // STUDENT WORK...completed
        clear();  //deletes all data blocks
        
        // Clear the array of block pointers
        // STUDENT WORK...completed
        delete [] block_pointers;  //dletes the array of now NULL block pointers
    ...
    void deque<Item>::push_front(const value_type& entry)
    {
        ...
        if (first_bp == NULL)
        {
            ...
            // STUDENT WORK... completed
            *first_bp = new Item[block_size];   //new block
            last_bp = first_bp; //point last at first block
            front_ptr = *first_bp;  //front and back_ptr point to first block
            back_ptr = *first_bp;
            *front_ptr= entry;  //first slot in first block set to entry
        ...
        else if ((*first_bp == front_ptr) && (first_bp != block_pointers))
        {
            // STUDENT WORK...completed
            //crete new block
            --first_bp;  //decrement first block ptr
            *first_bp = new Item[block_size];  //point it at new data block
            front_ptr = (*first_bp) + block_size - 1;   //point front_ptr at last spot of new bp
            *front_ptr = entry;     //set the front_ptr to entry
        }
    ...
    void deque<Item>::push_back(const value_type& entry)
    {
        ...
        if (last_bp == NULL)
        {
            ...
            // STUDENT WORK...completed
            *last_bp = new Item[block_size];   //new block
            first_bp = last_bp; //point first at last block
            front_ptr = *first_bp;  //front and back_ptr point to first block
            back_ptr = *first_bp;
            *front_ptr = entry;  //first slot in first block set to entry
        ...
        else if (back_ptr != ((*last_bp) + (block_size - 1)))
        {
            ...
            ++back_ptr;
            *back_ptr = entry;
            //simply move pointer up one and insert entry
        ...
        {
            // STUDENT WORK...completed
            ++last_bp;  //move block pointer and add block to that location
            *last_bp = new Item[block_size];
            back_ptr = *last_bp;    //back becomes first entry of new block
            *back_ptr = entry;
        ...
        else if ((back_ptr == ((*last_bp) + (block_size - 1))) && (last_bp == block_pointers_end))
        {
            // STUDENT WORK...completed
            reserve();
            ++last_bp;  //increment last block ptr
            *last_bp = new Item[block_size];  //point it at new data block
            back_ptr = *last_bp;   //point fback_ptr at first spot of last_bp
            *back_ptr = entry;     //set back_ptr to entry
    ...
    void deque<Item>::pop_front()
    {
        ...
        else if (front_ptr == ((*first_bp) + block_size - 1))
        {
            ...
            //std::cout << *first_bp << std::endl;
            delete[] *first_bp; //deletes block
            *first_bp=NULL; //set entry to NULL
            ++first_bp; //increment pointer
            front_ptr = *first_bp;
            //delete block, move pointer, set front_ptr
    ...
    void deque<Item>::pop_back()
    {
        ...
        if (back_ptr == front_ptr) //1 item case
        {
            clear( );
        }
        ...
        {
            // STUDENT WORK...completed
            //delete block with item in it since there is only 1 item is said blck
            delete[] *last_bp;  //delete data block
            *last_bp=NULL;  //asign NULL to location
            --last_bp;  //decrement pointer
