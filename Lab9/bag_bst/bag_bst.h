...
#pragma mark - Member Function Implementation
    ...
    // Header file used: bintree.h
    {
        ...
        if (this==&source)
            ...
            tree_clear(root_ptr);  //clear previous tree
            root_ptr = tree_copy(source.root_ptr); //copy source tree into this root ptr
        }
    }
    ...
    bag<Item> operator +(const bag<Item>& b1, const bag<Item>& b2)
    {
        // STUDENT WORK...completed
        bag<Item> sumBag;
        sumBag.insert_all(b1.root_ptr);
        sumBag.insert_all(b2.root_ptr);
