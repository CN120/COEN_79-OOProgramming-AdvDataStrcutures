...
namespace coen79_lab7
{
    ...
    void list_tail_insert(node* &tail, const std::string &newName, const float &newPrice) {
        ...
        assert(tail!=NULL && newName.length() > 0); //checks that node is present and newName is not empty
        node* last = new node(newName, newPrice, NULL);  //new node
        tail->setLink(last);    //tail's link = new node
        tail = tail->getLink();  //set tail to new node
    ...
    void list_head_remove(node*& head) {
        // IMPLEMENTATION COMPLETED
        node* delete_ptr = head; //temp pointer to head
        head = head->getLink();  //set head to its link
        delete delete_ptr;      //delete original head pointer
    }
